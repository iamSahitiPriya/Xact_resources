...
resource "aws_ecs_task_definition" "xact-task-def" {
  ...
  container_definitions    = jsonencode([
    {
      name      = "xact-task-def-qa"
      image     = "730911736748.dkr.ecr.ap-south-1.amazonaws.com/xact-qa:latest"
      cpu       = 1024
      memory    = 2048
      essential = true
      environment : [
...
resource "aws_iam_role" "xact-backend-role-ecs" {
  name = "xact-ecsTaskExecutionRole-qa"
